...
| **Exported engine templates** | The output the add-on writes per target — e.g. the Godot `lpc_shader.gdshader` + `lpc_material.tres`, generated from the templates in `addon/export/<target>/` (currently `addon/export/godot/`). | **MIT** | [addon/export/godot/LICENSE](addon/export/godot/LICENSE) |
